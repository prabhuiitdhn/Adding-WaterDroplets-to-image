...
# Input image
path = "D:/Camera Blockage/implementation/test_image/1557139659578560.png"
PIL_bg_img = Image.open(path)
bg_img = np.asarray(PIL_bg_img) #image as array
# to check if collision or not
label_map = np.zeros_like(bg_img)[:, :, 0] # label map array
...
# add Droplets object for each number of droplets.

for i in range(total_number_of_droplets):
    radius = random.randint(30, 50)  # randomly generating the radius for an
    position = ran_pos[i]
...
for drop in listFinalDrops:
    ...
    alpha_map[iy - ROI_HU:iy + ROI_HD, ix - ROI_WL: ix + ROI_WR] += drop_alpha[3 * radius - ROI_HU:3 * radius + ROI_HD,
...
for drop in listFinalDrops:
    ...
    PIL_bg_img.save('D:/Camera Blockage/implementation/output.jpg')
    cv2.imwrite("D:/Camera Blockage/implementation/output_label.jpg", np.array(output_label) * 255)
    # sys.exit()
